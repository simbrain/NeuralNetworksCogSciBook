...
% "Perhaps the variation between individuals of a species is of the same kind as the variation between different instances of the same identification model."  Learning finds a viable model. "Given the conjecture that all the possible models are similar, we might expect that any unbiased optimization procedure would generate about the same set of models. Thus, to a first approximation, the final results should not be dependent on the learning procedure. In particular, it should not be necessary to mimic biological development and learning to get veridical models." "correct results do not require realistic learning procedures that simulate the way the brain develops and adapts."  "The neural systems identification paradigm provides a systematic way to generate realistic models starting with a high-level description of a hypothesized computation and some architectural and physiological constraints about the area being modeled....This approach eliminates the need to find neural implementation algorithms by ad hoc means, because it uses optimization techniques that work automatically once the problem has been set up." Then we add constraints. "additional physiological constraints can be added until the model behavior mimics the experimental data."
...
One way to understand how these representations work is that they involve remapping and recombining patterns in the input space  in various ways. In some cases, as in the one shown in the figure, separate clusters get merged at another layer.  We also saw this in the discussion of Selfridge in chapter \extref{ch_history}, how different layers will combine representations of earlier layers to produce complex features. The same idea also comes up in the ``stacked'' layers of a deep network (chapter \extref{ch_cnn}).
...
% Generally start by training a network to achieve some task. The Zipser material can be used for this.
% Details on how they are validated
% Psychological tests: priming, latency effects. IAC and Seidenberg and McClelland from early chapter
% Similar responses properties (deep nets below) and receptive fields (zipser)
% Meaningful regions of activation spaces
% Similar receptive fields
% Similar behavior under ablation.  See https://link.springer.com/article/10.1007/s42113-020-00081-z/tables/1
% Similar behavior under stimulation
% RSA and other matrices like dissimilarity matrices (basically distance, where higher is more dissimilar). See Yamins 2014.
% Use one model to study another: https://openai.com/research/language-models-can-explain-neurons-in-language-models
% Cohere with mechanistic interpretability

\section{Net Talk}


% 7×29 one-hot letters; outputs = 21 articulatory features + 5 stress/syllable-boundary units. No reason to have separation in the inputs since it's just one hot letters.  But they did test with no hidden layer. "With no hidden units, the performance rose quickly and saturated at 82% best guesses as shown in figure 7a. This represents the part of the mapping that can be accomplished by linearly separable partitioning of the input space (13)... The pattern of errors was different from that observed in networks with a layer of hidden units in that many were stereotyped and inappropriate. Hidden units allow more contextual influence by recognizing higher-order features amongst combinations of input units"
NETtalk (1987) \cite{sejnowski1987parallel} was a model of reading English words aloud, created by Terrence Sejnowski and Charles Rosenberg in 1987. The network is a three-layer feed-forward network with the structure shown in figure \ref{net_talk}. The input layer codes for a moving window of letters, one of which is taken to be the current input and  the rest of which provide the network with information about neighboring letters.\footnote{Each letter is coded by a 29-dimensional vector (involving one-hot, one-of-29 representations of particular letters, with three additional nodes representing punctuation and word boundaries).} The hidden layer has 80 hidden units. The output layer has 26 units: 21 encode articulatory features of phonemes, such as place of articulation, voicing, and vowel height, and five encode stress and syllable-boundary information.\footnote{An interacting recreation of this simulation is in Simbrain, in the \simref{Language} submenu of Simbrain's \simref{Simulations} menu as \simref{NETtalk}, and see the related speech synthesizer documentation at \weburl{https://docs.simbrain.net/docs/worlds/speechsynthesizer.html}.} It was presented with written letters in English and was trained to pronounce those letters. The standard network had 18,629 weights and biases. In one dictionary experiment, a 120-hidden-unit variant trained on the 1000 most common words in a 20,012-word dictionary generalized to the full dictionary at 77 percent accuracy without additional training; with continued training on the dictionary, performance reached 90 percent after five passes. The network was shown to perform well with noisy input and to gracefully degrade \cite{sejnowski1987parallel}.
...
\begin{figure}[h]
\centering
\accessibleimage[width=0.8\textwidth]{images/netTalkCategories.png}{NETtalk architecture maps seven input-letter groups through 80 hidden nodes to 26 output nodes. Its hidden-unit states form separate clusters for vowels and consonants; the central input letter n maps to the consonant cluster.}
\caption[Pamela Payne and Jeff Yoshimi.]{NETtalk models convert written words to sounds, \ie reading aloud. Each letter group has 29 nodes (not the 3 shown). The 26 output nodes encode a distributed speech code, with 21 articulatory-feature units and five stress or syllable-boundary units. When trained using backprop, it developed internal representations of vowels and consonants, without having been told about them. The idea is schematically shown here. The current central input is an ``n'', a consonant, and so the hidden-unit state would correspond to one of the points in the consonant cluster.}
\label{net_talk}
\end{figure}
% This needs pictures of the vowel vs. consonant part of its state space. That dimension of the discussion is being left out.
% Somehow convey the 29 nodes per group. Maybe just draw them out?  Or use ellipses

One striking feature of this example is that the sounds the networks produced could be artificially synthesized and played back. You can ``hear'' the network learn (listen for yourself at  \weburl{https://www.youtube.com/watch?v=gakJlr3GecE}). Initially the network produced a babble of sounds, like a baby, but it eventually produced reasonably fluent (albeit somewhat creepy) speech \cite{sejnowski1987parallel}. So again, even if backprop is not neurally plausible, it seems to be doing something \emph{like} what the brain does, and it does so by developing psychologically realistic representations of vowels and consonants.
% See the semantic cognition book.

\section{Elman's Prediction Networks}

%  Mention Simbrain sims (maybe with a screenshot)

Another early-connectionist analysis of internal representations occurred in the context of Jeff Elman's simple recurrent networks (see chapter \extref{ch_supervised_recurrent}). In a famous paper \cite{elman1990finding}, Jeff Elman trained a simple-recurrent network like the one in figure \ref{elman_srn_categories} to predict the next words in a sentence.\footnote{\weburl{http://psych.colorado.edu/~kimlab/Elman1990.pdf}.}  The input data used to train the network consisted of thousands of sentences generated using a simple ``caveman'' grammar. Some sample sentences generated by this grammar are shown in figure \ref{elman_sentences}. The network predicts the next word in  a sentence at any time. For example, it predicts that the word ``cat'' will be followed by ``chase'' or ``eat''. With training, error can be reduced, but it never goes to zero, because a given word can be followed by more than one other word. Some words are more likely than others to follow one another, and the predictions the network makes match these patterns. It also learns some general rules, like expecting verbs after nouns \cite{elman1990finding}.\footnote{A Simbrain simulation that partially replicates the results of these papers can be accessed in the \simref{Language} submenu of Simbrain's \simref{Simulations} menu as \simref{Next-word prediction (SRN)}.}

\begin{figure}[h]
\centering
\frame{\accessibleimage[width=0.65\textwidth]{images/elman_sentences.png}{Two columns of simple training sentences for Elman's word-prediction network, such as mouse move book, cat move book, woman break, and monster smash plate.}}
...
After being trained to predict the next word in a simple text corpus (using the same types of \glossary{auto-regression}'used in transformers like GPT; see chapter \extref{ch_transformers}), the network's internal states were studied. After training, the average response of the network to different words was tested, and it was found that similar words were near each other in the hidden-unit space. This can be seen in figure \ref{elman_srn_categories}, which can be thought of as a dimensional reduction of the hidden-unit space.\footnote{The original picture this was adapted from was generated by exposing the network to each word in the context of many sentences (\eg ``eat'', ``cat''), and then taking the average hidden-unit activation across these exposures. The resulting vectors are like the centers of clusters in the hidden-unit space.}  Each point in the right panel of the figure correspond to a vector (an activation pattern) in the hidden-unit space, projected to two dimensions. Distances between points are meaningful: points closer to each other in the diagram correspond to hidden-unit vectors that are closer to each other in the hidden-unit space.

\begin{figure}[h]
\centering
\accessibleimage[width=0.8\textwidth]{images/elmanCategoriesSpace.png}{Elman simple recurrent network and its learned hidden-state categories. The network predicts a next word from the current word and context; projected hidden states cluster into nouns versus verbs, then animate versus inanimate nouns and several verb subcategories.}
...
% Also V4. Discuss a bit about how V1, V2, V4 different. Cite other papers that discuss other areas. Mention Yamins and his lab by name in main text.
...
\begin{figure}[h]
\centering
\accessibleimage[width=0.8\textwidth]{images/deepNetYaminsArchitecture_2016.png}{Three-panel comparison linking visual brain processing to a deep network. Image stimuli pass through visual areas and through successive convolutional layers; green dashed lines connect corresponding brain regions, including V1, V2, V4, and inferotemporal cortex, to network layers.}
...
\begin{figure}[h]
\centering
\accessibleimage[width=0.6\textwidth]{images/deepNetYaminsITResponses_2016.png}{Line graph comparing a deep network's hidden-unit activation in black with an inferotemporal neuron response in red across images. Both traces rise sharply for sample images containing a head, showing similar selectivity.}
...
% OTHER EXAMPLES
% Triangle model
%  \weburl{https://www.youtube.com/watch?v=gakJlr3GecE}
...
%\caption[From \weburl{http://cseweb.ucsd.edu/\~gary/258a/Backprop.pdf}.]{Optimal input pattern for a hidden (``Gestalt'') layer face node. Shows what kinds of inputs the unit will respond to.}
%\label{holon}
%\end{figure}
%
