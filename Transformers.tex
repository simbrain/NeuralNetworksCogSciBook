...
% Dividing not over the axis of the number of tokens but on the token embeddings. This is more computationally efficient.
Within each block all the mechanisms above are separated into multiple attention ``heads''.   Each head operates on a different subset of the embedding dimension.\footnote{To confirm this, note that in figure \ref{gptParams}, the number of inputs to each head, $d_\text{head}$, is equal  or almost equal to  the embedding dimension $d_\text{model}$ times the number of heads $n_\text{heads}$.}Each head is thus responsible for one \emph{part} of the vector representation of each word. In our simple example of a 4-d embedding (see figure \ref{contextWindow}), if there were two heads, each would handle two of the four numbers in each token embedding.  
