...
% Eric S: Re predictive processing. "I'm not so sure about bridging this over to predictive processing.  It's the latest hot thing, but I'm not sure this result specifically supports PP over any other approach that recognizes that people process sentences unidirectionally."
...
% Discuss encoders, decoders (the language is most basic case of this is auto encoder where we compress down to a latent space then decompress, go down to essential information, then make it bigger). Discuss masking.  BERT was bidirectional, but unidirectional was better for text generation.  Make it clear how BERT relates to all this. For example, we can feed a transformer network the first halves of hundreds of movie scripts and train it to produce the second halves of those scripts, or we can train it to predict what the first summary paragraph of a Wikipedia article is based on the main body of the article. 
...
Within each block all the mechanisms above are separated into multiple attention ``heads''.   That is, the $\textbf{K}$, $\textbf{Q}$, and $\textbf{V}$ matrices reduce from the embedding dimension to a head dimension.\footnote{This is roughly the number of embedding dimension divided by the number of heads so that when the outputs concatenated we get back to the same dimension. This can be seen in figure \ref{gptParams}, the number of inputs to each head, $d_\text{head}$, is equal  or almost equal to  the embedding dimension $d_\text{model}$ times the number of heads $n_\text{heads}$.}  For example if the heading dimension were 8 it might get reduced to three 3-dimensional heads.
