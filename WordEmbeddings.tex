...
\begin{figure}[h]
\centering
\accessibleimage[scale=.45]{./images/treeBank.jpg}{Syntactic parse tree for the sentence ``The house at the end of the street is red.'' The sentence branches into a noun phrase and verb phrase, with nested phrase and part-of-speech labels.}
\caption[From \weburl{https://commons.wikimedia.org/wiki/File:The_house_at_the_end_of_the_street.jpg}]{A treebank for the sentence ``The house at the end of the street'' which annotates each word with its syntactic or grammatical category and describes dependencies between parts of the sentence.}
\label{treeBank}
\end{figure}
...
Figure \ref{f:writerPainterExample} shows seven words embedded in a space. The example illustrates that the relative relations between tokens are preserved.\footnote{These were plotted using \weburl{http://vectors.nlpl.eu/explore/embeddings/en/}, which is a helpful interactive page for getting an intuitive feel for how word embeddings work.}  Notice that the creator of a medium (i.e., composer, author, painter) is closest to the medium or composition (i.e., music, book, painting). Interestingly, ``critic'' is closest to ``author''.

\begin{figure}[h]
    \centering
    \accessibleimage[scale=.2]{./images/Word_vector_demo.png}{Word-embedding scatterplot. Music is near composer, book is near author and critic, and painting is near painter, illustrating that semantically related words occupy nearby positions.}
    \caption[Adapted from an image generated using \weburl{http://vectors.nlpl.eu/explore/embeddings/en/}.]{Example of a word embedding in a semantic space. Each word is associated with a point in a space and the distance between points corresponds to semantic similarity. Notice that intuitively similar words are near each other in this space. }
...
An example is shown in figure \ref{simpleCoocmatrix}, based on the text ``cats chase mice quickly''.  The context window has a size of 1 and is bidirectional. Each pair of words that appear next to each other increases their co-occurrence count by one.
...
Other variants of this approach include FastText \cite{bojanowski2016enriching}, which utilizes subtokens (character n-grams that make up a word, such as `ca' and `at' for `cat') in its representations to handle words that it might not have encountered during training (out-of-vocabulary terms), and GloVe \cite{pennington2014glove}, which predicts \textit{ratios} of co-occurrence probabilities from global statistical information, instead of directly predicting co-occurrence probabilities from local context as in Word2Vec. Unlike Word2Vec's local context approach, GloVe uses global co-occurrence statistics across the entire corpus to learn embeddings.

Starting around 2018, contextualized embeddings became more common in the context of bidirectional language models such as ELMo and  BERT \cite{devlin2018bert} (on transformers, see chapter \extref{ch_transformers}). BERT uses the same basic architecture as a chat model like ChatGPT but is trained in a different way.\footnote{BERT and its variants are generally trained to predict a masked word in a context, for example, ``she poured the [MASK] into the mug'', rather than the next word in a sequence. This is called ``masked language modeling''.} After training, Word2Vec produces a set of static word embeddings, where each word corresponds to a single (high-dimensional) vector, regardless of the surrounding context. These static embeddings can be thought of as a dictionary that takes words as keys and maps them to vectors. BERT and its variants, on the other hand, can produce dynamic word embeddings that are sensitive to the surrounding context. In contrast to Word2Vec pretrained embeddings, BERT is used as a pretrained language model. In real usage, one will provide BERT with a context sentence and then extract out a contextualized word embedding. This makes it possible to distinguish different meanings of words, for example ``bank'' in the context of a river vs a financial transaction. Here's a simplified example of what a function call to this type of model might look like. Notice that in this case, the request has \emph{two} arguments: a sentence, and a target-word:
...
\begin{figure}[h]
\centering
\accessibleimage[scale=0.4]{./images/kingQueen.png}{Two vector diagrams show analogy directions in a word-embedding space. Parallel arrows link man to woman, uncle to aunt, and king to queen; a second panel similarly links singular nouns to plural forms.}
