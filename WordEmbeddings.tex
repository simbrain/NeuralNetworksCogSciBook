...
% The phoneme level is the level of sounds like ba and pa often represented in the IPA (citation). One way this level is used is to generate representations of human sounds in vector form. This can be done in a very simple way, as in Elman (see commented out below). It can also be done using spectrograms. A spectrogram is a plot that shows time on the x-axis and "amount" of frequency on the y-axis. 

% Morphology. This is word parts like hunt vs. ed in hunted. Not used much directly in terms of embeddings though a bit in tokenization.  Neural network models of paste tense transformation were big. They could learn standard rules quickly  like hunt to hunted but the exceptions take longer, as in humans.

% Syntax. This is rules of how you can put words together like... Here are automated methods used. Explain all terms like treebanks. Brief.  In neural networks syntax has been the center of a giant debate. Chomsky has claimed syntax is an innate module that can;'t be learned in a neural network given the poverty of the stimulus. Neural networks require large training examples and often are not good at dealing with the same range of phenomena as classical tree-structure approaches. The debate remains healthy! Citations.
...
% Pragmatics is implicit rules for language use.. Grice is well known.  Not studied much in neural networks but become important with LLMs since they can have actual conversations. See llm chapter
...
% TODO: refs above
% TODO: Revise above. We will just look closely at cooccurrence based embeddings based on smaller corpora. In those cases the columsn are the same as rows and sow the have a meaning, the word. But in larger corpora dimensionality reduciton like SVD is used and so the columsn are not meaningful. 
% If there's a way to "..." for more rows do that
...
Not every word is used with the same frequency. Similar to our document embeddings, some determiners (`the', `a') may be over-represented and skew the co-occurrence matrix. One approach to the issue is to simply filter out these words, which are sometimes called  \emph{stopwords}.
...
% Another kind of embedding is a simple one that is just random vectors that we then train with gradient descent.
...
% TODO  A picture showing a string of words, integers above them, and then rows of an embedding matrix, and then the final matrix representation of the document
% Important to cohere this with transformer chapter. Show in particular how word embeddings can themselves be trainable using gradient descent. 
