...
This trains a network to produce associations between a one-hot representation of a word and a probability distribution over context words (a softmax activation function is used on the output to obtain probabilities). The hidden layer has far fewer dimensions than the input and output layers, so that we get a kind of compressed representation of the task of associating words with their context. After the network is trained on this task, activations in the hidden layer can be used as word embeddings The number of nodes in the hidden layer corresponds to the embedding dimension of the embedding Word2Vec produces.\footnote{The embedding dimensions are typically around 300D, but newer models might exceed 700D.}  So the network is trained to predict context, but in the process of doing so it develops a useful word embedding at its hidden layer. 
...
Starting around 2018, embeddings became more common in the context of transformer models such as BERT and ELMo (on transformers, see chapter \extref{ch_transformers}). These models were often trained to predict a masked word in a context (``masked language modeling''). This is in contrast to how familiar models like ChatGPT are trained (to predict next words from prior context using self-supervised learning; see  chapter \extref{ch_transformers}). As with Word2Vec, by training these models on this task, the network develops useful embeddings. 

BERT and its variants are still used to produce embeddings for words in context. Whereas Word2Vec produces a fixed association between a word and a vector, BERT produces an embedding for a word in the context of a whole sentence.

%In earlier models, like Word2Vec and GloVe, the whole point of the network was to produce the embedding that captured semantic or some type of linguistic information. But with modern transformers, the goal is to create a pre-trained model that can be adjusted to a particular task or use-case, and the embedding is just part of what gets trained overall as part of one big model. The embedding serves to enable complex functionality like chat, which relies on understanding core semantic concepts. The value of this kind of (pre-trained?) custom trained embedding is that it ends up being well-suited to whatever task (for example next word prediction) the network is trained on.\footnote{It is worth noting that while traditional Word2Vec and GloVe embeddings are still used for particular research projects, they have largely fallen out of favor relative to transformer models. Modern language models train embeddings end-to-end as part of their overall architecture, and given the fact that these models can generate contextualized word embeddings to capture context-specific word meanings, these newer transformer models have become the first step for most embedding needs.}
...
% Remove graphic example; say it in words; and point to llm chapter
...
Now we can take each integer and associate it with a row of an embedding matrix, like this (the example is shown with integer labels on rows to make the idea clear):
\[
