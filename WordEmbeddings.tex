...
% BERT, discussed in transformer chapter briefly, is also a very complex embedding, that is associated with the rise of transformers.  Not just word embedding but also sentence embedding.  Was trained to find a masked word (not next word).  What comes out is never  a simple token embedding but an embedding of a word in a sentence. Captures meanings of words in a sentence.  Was a new efficient way to embed a sentence. A way to get a short representation of text to summarize it, map it to images or videos etc. 
