...
\chapterauthor{Jeff Yoshimi, Scott Hotton}
  ...
  In chapters \extref{ch_act_functions}, \extref{ch_linear_algebra}, and \extref{ch_supervised}, we have noted that when the ``play''  button \accessibleartifact[scale=.5]{./images/Play.png} is pressed in Simbrain a dynamical process is simulated; neurons start firing and changing, weights will sometimes change their size, etc.
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.45]{./images/StateSpaceParts.png}{Phase portrait in a square two-dimensional state space. Arrows trace several orbits from initial conditions toward one central attracting fixed point.}
...
A \glossary{state} of a system is a specification of values for all of the state variables which describe that system. If we model water using temperature (Fahrenheit) and volume (Liters) as our state variables,  a state for the pot of water might be $(89.8, 2)$. If we model a person using height (inches), weight (pounds), and blood sugar (mg/dl) as our state variables, a  state for the person might be $(60,150,75)$. A state for the nodes or the weights of the network is a large vector or matrix that is too long to write out here. Dynamics then describe how these states---e.g. activation vectors, weight matrices, or other collections of state variables---change in time.

The state of the network in figure \ref{F:hopfieldStateSpace} is $(-.8,.8)$, which are the values of two state variables $a_1$ and $a_2$, corresponding to the activations of the two nodes. However, recall that what we take to be a ``state'' of a system is up to us. So instead of looking at node activations, we could have looked at weight strengths. Then the state variables are $w_{1,1},w_{1,2}$ and the current state is  $(-1,-1)$.

% The basin picture is misleading because the circle is not around a basin of attraction
\begin{figure}[h]
\centering
\raisebox{-0.5\height}{\accessibleimage[scale=.2]{./images/HopfieldNetAttractor.png}{Two-node recurrent Hopfield network. A blue node with activation minus point eight connects reciprocally to a red node with activation point eight.}}
\hspace*{1in}
\raisebox{-0.5\height}{\accessibleimage[scale=.6]{./images/HopfieldPortrait.png}{Phase portrait for the two-node Hopfield network. Orbits spiral toward two attractors near minus point eight, point eight and point eight, minus point eight, while the origin is a saddle point.}}
...
% Add numbers for vertical axis
\begin{figure}[h]
\centering
\raisebox{-0.5\height}{\accessibleimage[scale=.15]{./images/Pendulum.png}{Simple pendulum: a bob suspended from a pivot swings through an arc on either side of vertical.}}
\hspace*{.8in}
\raisebox{-0.5\height}{\accessibleimage[scale=.2]{./images/PendulumPortrait.png}{Pendulum phase portrait. Curved orbits in angular-displacement and angular-velocity space spiral inward toward the resting state at zero, zero.}}
\caption[Left: From \weburl{https://commons.wikimedia.org/wiki/File:Simple_gravity_pendulum.svg}; Right: Scott Hotton.]{Pendulum (Left) and its state space with a phase portrait (Right). The vertical dimension of the state space corresponds to angular velocity; the horizontal dimension corresponds to angular displacement away from hanging straight down.}
...
A pendulum is a classical example of a dynamical system. Pendulums have been studied extensively ever since Leonardo da Vinci designed fairly accurate clocks based on them. The state of a pendulum is given by two variables, one variable $\theta$ for the angular displacement of the pendulum from verticality and another variable $\dot{\theta}$ for the angular velocity.\footnote{Note the dot over $\dot{\theta}$, which indicates the first derivative of $\theta$, i.e. rate of change of angular displacement, which is angular velocity.} If we start a pendulum with some chosen values for these two variables we can, at least in principle, say exactly how it will move forever in the future. Figure \ref{F:pendulumStateSpace} shows a pendulum on the left and its state space on the right. The horizontal axis shows the angular displacement, $\theta$, and the vertical axis shows the angular velocity, $\dot{\theta}$. We can put the bob of the pendulum in any initial position and give it a push with any initial angular velocity and the future of the pendulum will be determined. This behavior can be predicted by choosing the corresponding point in the phase portrait and following the orbit through that point. Eventually, because of friction, the orbit will close in on the point $(0,0)$ which corresponds to the pendulum hanging straight down without moving.\footnote{The state space is actually an infinitely long cylinder. Imagine wrapping the left and right edges of the state space in figure \ref{F:pendulumStateSpace} around and gluing them together.}
...
These sudden shifts in the behavior of a system when parameters are changed are called bifurcations. A \glossary{bifurcation} is a radical (more precisely, ``topological'') change in the phase portrait of a dynamical system that occurs when the parameters are changed past certain critical values (these values are sometimes called ``critical points''). An example of a bifurcation is shown in figure \ref{F:bifurcation}. In that figure, a single point (left) gives rise to a circle (right) when the parameters are changed past a critical value.

\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/bifurcation.png}{Hopf bifurcation comparison. At left, trajectories spiral into a stable fixed point; at right, the fixed point repels and trajectories approach a stable circular orbit.}
...
In this section, we  introduce some language for classifying orbits and using them to understand the overall behavior of a system. The basic categories we consider are shown in figure \ref{F:attractorsRepellers}.

\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/AttractorsRepellers.png}{Four phase portraits: an attracting fixed point, a repelling fixed point, an attracting periodic orbit, and a repelling periodic orbit. Blue trajectories approach attractors and leave repellers.}
\caption[Pamela Payne.]{An attracting fixed point (upper left), repelling fixed point (upper right), attracting periodic orbit (lower left), and repelling periodic orbit (lower right). Attractors / repellers are shown in black; transient orbits approaching or leaving attractors and repellers are shown in blue.}
...
top row of figure \ref{F:attractorsRepellers}.  This type of orbit is just a
...
of figure \ref{F:attractorsRepellers}.\footnote{The Poincar\'{e}-Bendixson
...
\begin{figure}[h]
\centering
\accessibleimage[scale=0.1]{./images/Two5-cycles.png}{Two discrete five-cycles. Open circles connected by directed dashed arrows show five states visited in sequence; the left cycle is a pentagon and the right is an irregular crossing path.}
...
\begin{figure}[h]
\centering
\accessibleimage[scale=0.333]{./images/Attracting5-cycle.pdf}{Attracting five-cycle. Directed paths from nearby filled points move toward the five open-circle cycle.}
\accessibleimage[scale=0.333]{./images/Repelling5-cycle.pdf}{Repelling five-cycle. Directed paths from nearby filled points move away from the five open-circle cycle.}
\caption[Scott Hotton.]{The $5$-cycles are shown as in the left panel of figure
...
the bottom row of figure \ref{F:attractorsRepellers} are invariant sets.  They
are the union of spiral-shaped orbits (not all of which are shown) and a fixed
point.  Invariant sets can have many other shapes and extend into higher-dimensional spaces.  For example a two-dimensional torus in a three-dimensional
state space can be an invariant set.
Invariant sets can even be fractals like the one shown in figure \ref{F:chaoticStateSpace}.
...
given attractor. This is useful because we can then partition a state space into basins, one for each attractor. If you start a system off anywhere in the basin of attraction for an attractor eventually it must end up on or very close to that attractor.
...
comes to rest at and the whole valley is its basin of attraction. The state space in figure \ref{F:hopfieldStateSpace} has two attractors with two basins of attraction.
% Talk about basin boundaries, separatrices, etc.
\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/BasinAttraction.jpeg}{Hill-and-valley landscape representing basins of attraction. Red balls on a wavy surface roll toward low points in separate valleys.}
\caption[From \weburl{http://www.scholarpedia.org/article/File:Hopfieldattractor.jpg}. Licensed Under CC BY-NC-SA]{Attractors and basins of attraction pictured using a hill and valley
metaphor.}
\label{F:basins}
\end{figure}
...
There are orbits that are neither repellers nor attractors. For example, the central point in figure \ref{F:hopfieldStateSpace} is attracting in one direction, and repelling in another.

\subsection{Combining these classifications}

Combining the results of the last two subsections, we can classify the orbits of a dynamical system in terms of the topology of its orbits \emph{together} with whether they are attracting or repelling. This classification is evident in figure \ref{F:attractorsRepellers}. Here is the same classification in table form. In the table below, the columns correspond to different topologies or shapes  that an orbit can have. The rows correspond to the behavior of states nearby the orbit.
