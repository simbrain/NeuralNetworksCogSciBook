...
A \glossary{tensor} generalizes vectors and matrices to arrays with any number of indices. A scalar is a rank-0 tensor, a vector a rank-1 tensor, and a matrix a rank-2 tensor. A stack of matrices, or volume, is rank 3. The \glossary{shape} of a tensor is a list of numbers in parentheses that reports the size along each array dimension. We already saw shape with matrices: a matrix with 2 rows and 3 columns has shape $(2,3)$, also written $2\times3$. An RGB image involves 3 color values for each pixel.\footnote{RGB is a way to represent an image. RGB stands for red, green, and blue. Each pixel's color can be represented by three values giving the amounts of these color components. See \weburl{https://en.wikipedia.org/wiki/RGB_color_model}.} An RGB image with 25 rows and 25 columns has shape $(3,25,25)$: three color channels, each $25\times25$. A \emph{batch} is a group of examples processed together, for example when training a network (see chapter \extref{ch_data_science}). A batch of 100 such RGB images has shape $(100,3,25,25)$.
...
The activation-function and linear-algebra questions below are adapted from the exercises in the original chapters, with additional questions on shapes.
...
\item \textbf{Piecewise linear.} Suppose node $k$ has a piecewise linear activation function with $(u,\ell)=(1,0)$. What is its activation if $n_k=0$, $n_k=0.5$, or $n_k=10$?

\item Suppose node $k$ has a piecewise linear activation function with $(u,\ell)=(2,-2)$. What is its activation if $n_k=0$, $n_k=-10$, or $n_k=10$?

\item \textbf{ReLU.} Suppose node $k$ has a ReLU activation function. What is its activation for each weighted input $n_k=-10$, $n_k=0$, and $n_k=19$?

\item \textbf{Sigmoid.} Suppose node $k$ has a sigmoid activation function with $(u,\ell)=(1,0)$. What is its activation if $n_k=0$? Approximately what is it if $n_k=-45$ or $n_k=104$?

\item Suppose node $k$ has a sigmoid activation function with $(u,\ell)=(1,-1)$. What is its activation if $n_k=0$? Approximately what is it if $n_k=-45$ or $n_k=104$?

\item Suppose node $k$ has a sigmoid activation function with $(u,\ell)=(10,0)$. What is its activation if $n_k=0$? Approximately what is it if $n_k=-45$ or $n_k=104$?
...
\item \textbf{Softmax.} Three nodes have logits $(3,2,1)$ and temperature $T=1$. Which node will have the highest probability? How will the probabilities change if the temperature is increased to $T=3$ or decreased to $T=0.1$? \textbf{Advanced:} Compute the probabilities at $T=1$ to two decimal places.
...
\item What is the dimensionality of the weight space and activation space of a two-node recurrent network with all four possible connections, including a connection from each node to itself?
...
\item Plot the points $(1,2)$, $(1,3)$, $(4,2)$, and $(-3,-2)$. Which pairs share a first component? Which share a second component? How does this appear in the plot?
...
\item \textbf{Weight-matrix shapes.} A feed-forward network has 4 input nodes, 6 hidden nodes, and 2 output nodes, with all-to-all connections between consecutive layers. Using the conventions in the chapter, what are the shapes of its input-to-hidden and hidden-to-output weight matrices? How many weights does each matrix contain?

\item \textbf{Reading recurrent weight indices.} Use the four-node matrix $\mathbf{W}_r$ accompanying figure \ref{abridged_sparse_recurrent}. What are $w_{1,4}$, $w_{4,1}$, and $w_{4,4}$, and which connection does each describe? List the index pairs $(j,k)$ for all nonzero entries. Which entry represents a self-connection?

\item \textbf{Recurrent weight matrix.}\label{abridged_ex_recurrent} A recurrent network has three nodes labeled $1,2,3$ and just three weights: $w_{1,2}=-1$, $w_{2,3}=0.5$, and $w_{3,1}=2$. What is the matrix representation of its weights using the conventions in the chapter?

\item \textbf{Image shapes.} Using the conventions in the chapter, give the shape and rank of a single RGB image with 25 rows and 25 columns. How would its shape change if it were grayscale?

\item \textbf{Batch shapes.} A batch contains 20 RGB images, each with 28 rows and 28 columns. Give its tensor shape using the conventions in the chapter.

\item \textbf{Tensor indexing.} Let $\mathbf{X}$ be a batch of RGB images with shape $(20,3,28,28)$. Use the conventions in the chapter. Describe exactly what the scalar entry $X_{7,2,10,12}$ represents. Write the indices for the blue intensity at row $4$, column $9$ of image $5$. If we select image $7$ and then its green channel, what is the shape of the remaining array?

\item \textbf{Reading a shape.} A tensor has shape $(60000,1,28,28)$ using the conventions in the chapter. How many images does it contain? Are they grayscale or RGB? How many rows and columns does each image have? What shape would a batch of just 100 of these images have?

\item \textbf{Distance in 3 dimensions.} Consider the points
\[
A=(0,0,0),\quad B=(0,0,1),\quad C=(0,0,10),\quad D=(8,8,0).
\]
Which pair is closest together in the original 3 dimensional space?

\item Consider the points
\[
A=(1,1,0),\quad B=(1,2,0),\quad C=(1,1,20),\quad D=(9,9,20).
\]
Which pair is closest in 3 dimensions?

\item \textbf{Distance in 4 dimensions.} Consider the points
\[
A=(0,0,0,0),\quad B=(0,0,0,1),\quad
C=(0,0,10,10),\quad D=(7,7,10,10).
\]
Which pair is closest in the original 4 dimensional space?
...
\item \textbf{A forward pass.} A feed-forward network has two weight layers:
\[
\mathbf{W}_{input,hidden}=\begin{pmatrix}1&.7&2\\-2&-1&2.1\end{pmatrix},
\qquad
\mathbf{W}_{hidden,output}=\begin{pmatrix}1&-1\\0.5&2\\-1&1\end{pmatrix}.
\]
For $\mathbf{a}_{input}=(1,2)$, what is the hidden activation vector with zero biases and no activation function? What is it with ReLU?

\item For the same network, suppose $\mathbf{a}_{hidden}=(1,0,2)$. What is the output activation vector with zero biases and no activation function? What is it with ReLU?

\item Starting from $\mathbf{a}_{input}=(1,2)$, what are the hidden and output activation vectors after propagating through both weight layers with zero biases and no activation functions?

\item Repeat the full forward pass with bias vectors $\mathbf{b}_{hidden}=(4,0,-1)$, $\mathbf{b}_{output}=(1,-2)$, and ReLU in both the hidden and output layers.

\item \textbf{Recurrent dynamics.} Let
\[
\mathbf{a}(0)=\begin{pmatrix}1&1&1\end{pmatrix},
\qquad
\mathbf{W}=\begin{pmatrix}0&-1&0\\0&0&0.5\\2&0&0\end{pmatrix}.
\]
What is the activation vector after one time step with zero biases and no activation function? What is it with ReLU?

\item For the same recurrent network, let $\mathbf{a}(0)=(-1,-1,2)$. What is the activation vector after one time step with no activation function? What is it with ReLU?

\item Starting again from $\mathbf{a}(0)=(1,1,1)$, what are the activation vectors after each of four time steps with no activation function? Repeat with ReLU.
\end{enumerate}
