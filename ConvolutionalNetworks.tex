...
Following the convention from chapter \extref{ch_linear_algebra}, tensor shapes are given in the order (channels, height, width). Recall that for matrices, height corresponds to the number of rows and width to the number of columns. Ignoring padding, the formula for width and height of the output tensor relative to input tensor and filter is:
\begin{align*}
...
As with convolutional layers, we can specify a stride for pooling operations. The formula for computing output shape from input shape is analogous to the convolutional case.

An example is shown in figure \ref{maxPooling}. The pool shape is $2 \times 2$ and the stride is $2$, so the window is passed over the input 2 at a time (notice that the right-most column gets left out in this case; a stride of 1 would fix that, or some padding).

For these operations we use the same equations as above, but with pool width and height. As with convolutional layers, there is no depth parameter in these formulas because pooling operates only on the spatial dimensions.
