...
\begin{figure}[h]
\centering
\accessibleimage[width=0.55\textwidth]{images/vectorLMSBeforeTrain.png}{Three-output, two-input network before vectorized LMS training. Input vector zero, one produces all-zero output because every weight is zero; the target output is one, zero, one.}
...
\begin{figure}[h]
\centering
\accessibleimage[width=0.55\textwidth]{images/vectorLMSAfterTrain.png}{The same network after one vectorized LMS update. The learned weights map input vector zero, one to the target output vector one, zero, one.}
