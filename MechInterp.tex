...
supervised learning (chapter \extref{ch_supervised}); they can be used when we
...
\cite{burns2024discovering} for another significant unsupervised approach.} With
sparse autoencoders, we do not need to know in advance what concepts might
appear represented in an activation space to find them. Nodes in an LLM tend to
be uninterpretable, seemingly each dealing in many unrelated concepts (this is
...
\glossary{activation addition}. Unlike linear probes (which train a simple
...
\begin{enumerate}
      ...
      \extref{ch_linear_algebra}) and at a later time add it back to that
