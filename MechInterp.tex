...
% On the use of linear probes to study syntax etc. "This led to the "probe wars" around 2019–2022. The modern view is that successful probing shows information is encoded in the representation, but not necessarily causally used by the network. That's one reason mechanistic interpretability became so important afterward." Should these probe wars be added as an intermediate stage?
% A useful mechinterp explainer and glossary: https://dynalist.io/d/n2ZWtnoYHrU1s4vnFSAQ519J
% Also: https://transformer-circuits.pub/2025/attribution-graphs/biology.html
...
thread at \weburl{distill.pub},
\weburl{https://transformer-circuits.pub/2021/framework/index.html}. Also see the more comprehensive thread with historical information at \weburl{https://transformer-circuits.pub/}} 
...
\weburl{https://youtu.be/veT2VI4vHyU}.}
\end{quote}
...
``circuit'' was in terms of graph subnetworks: the idea was that a circuit in a
model is the minimal set of units and edges that reconstructs a behavior. 
Behind this definition is the implicit assumption that
...
has not been successful at decomposing models into constituent
...
demanded.
% Is localization really an assumption of the graph network approach. Also who makes that assumption?

% Is the claim below true? Isn't mechinterp an end in itself, separately from value alignment?
...
fully examine an LLM's world model and make sure there is nothing nefarious in
...
\weburl{https://en.akinator.com/} to get an intuitive feel for how few binary
...
\begin{figure}[ht]
\centering
\accessibleimage[scale=.5]{./images/linearProbe.png}{A linear probe inserted into a transformer residual stream. The stream passes through attention heads and an MLP while a branch feeds a probe, depicted as an electrode, to classify the activations.}
...
% see new language in pierre/ jeff paper
% maybe add simbrain here too, since it will have no hidden layer and just be a single set of nodes (or two layers, if we re-represent the input). If we use a single set then change the sae pic so there are no hidden nodes
We harvest activations from somewhere in a transformer, often in
...
activations we collected, we know whether the feature was present or
absent, and use that knowledge to fix our training labels. For example, suppose we feed these simple ``caveman'' sentences through an LLM:
\begin{quote}
``The dog barked.''\\
``The red car stopped.''\\
``Alice ran home.''
\end{quote}
For each word in each sentence we know its part of speech (ignoring subword tokens for this example), and so we can extract a hidden-state vector corresponding to it, and train a linear probe to respond with the appropriate part of speech, classifying ``dog'' as a noun, ``bark'' as a verb, etc.  It is now hoped that this ``probe'' model will successfully generalize its
classifications to any future model activations.  In the example above, the noun node should activate whenever a noun token occurs, and similarly for the verb node, adjective node, etc.

Probes in a transformer are
...
%there is a challenge involved in selecting positive and negative examples that unambiguously exemplify the target feature as opposed to its many correlates: a collection of inputs mentioning politicians instantiate the feature “politician,” but the contrast class of negative examples needs to be carefully chosen to ensure that the probe does not learn to probe for the “political” feature, or even for the “person” feature, which are equally instantiated by the positive examples (see Gurnee et al., 2023).”

This extra work of training a probe is necessary because the transformer 
...
Thus, in LLMs as in human brains, an external process must be grafted onto the
...
are the same---it's an autoencoder---but we have trained the hidden unit
activations to be sparse. It is these few active nodes that we now focus our
interpretations on.

\begin{figure}[ht]
\centering
\accessibleimage[scale=.5]{./images/saeSimbrain.png}{Sparse autoencoder network in Simbrain. Input activations feed a larger hidden layer with mostly inactive nodes, then reconstruct matching output activations; colored node disks indicate activation values.}
\caption[Simbrain screenshot from Jeff Yoshimi.]{ A sparse autoencoder takes
      some input activations and maps them back to themselves (notice that the
...
Once an SAE has been built, it can be used to identify interpretable features. For example if a unit fires strongly for words like ``dog'', ``cat'', and ``horse'' we might suspect it is an animal feature. In practice, identifying and validating these features is labor-intensive, so researchers often rely on pretrained SAEs released by large research groups and explore their features using tools such as Neuronpedia.


% Add screenshots from neuronpedia and practical info about using SAE's
...
\glossary{activation addition}. While activation vectors in a model's neuron
...
that activation to blend that concept into behavior in new contexts
...
\begin{enumerate}
 ...
 is all done without revealing the microanatomy: we know \emph{which}
...
\begin{figure}[ht]
\centering
\accessibleimage[scale=.5]{./images/goldenGate.png}{Activation-steering example. A neural network diagram is paired with Claude responses: the default prompt yields a normal answer, whereas clamping a Golden Gate Bridge feature high makes the answer repeatedly mention the bridge.}
...
while keeping the rest of the network in its typical, ``healthy''
...
option is resampling ablations, in which case we take the \emph{actual
...
the sum of their angles is computed as a composition of two rotations (see figure
...
\begin{figure}[ht]
\centering
\accessibleimage[scale=.4]{./images/toyModelAddition.png}{Two unit-circle diagrams explain modular addition as rotations. Panel A shows rotations a and b; panel B shows their combined rotation a plus b, illustrating how a trained network represents addition.}
...
keep a lookout for key regularities in your context, and to redeploy that
