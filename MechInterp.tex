...
Once an SAE has been built, it can be used in to identify interpretable features. For example if a unit fires strongly for words like ``dog'', ``cat'', and ``horse'' we might suspect it is an animal feature. In practice, identifying and validating these features is labor-intensive, so researchers often rely on pretrained SAEs released by large research groups and explore their features using tools such as Neuronpedia. 
