...
``Alice ran home.''
\end{quote}
For each word in each sentence we know its part of speech (ignoring sub-word tokens for this example), and so we can extract a hidden-state vector corresponding to it, and train a linear probe to respond with the appropriate part of speech, classifying ``dog'' as a noun, ``bark'' as a verb, etc.  It is now hoped that this ``probe'' model will successfully generalize its
