...
% Outer product as a way of referring to column times row.

\subsection{Elementwise (Hadamard) Product}

The elementwise, or Hadamard, product of two vectors or matrices $\mathbf{a}$ and $\mathbf{b}$ with the same shape is denoted by $\mathbf{a} \circ \mathbf{b}$ and is defined as follows:

\begin{eqnarray}
(\mathbf{a} \odot \mathbf{b})_{ij} = \mathbf{a}_{ij} \odot \mathbf{b}_{ij}
\end{eqnarray}
That is, we multiply corresponding components of the two vectors to produce a new vector with the same shape as the original.  In a way this is the most intuitive concept of multiplying vectors or tensors. Just line them up and multiply.

For example, $(1,2) \odot (3,4) = (3,6)$.
