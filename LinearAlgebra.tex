...
% Add discussion of tensors and rank to set up deep net discussion

% Adding weight matrices. Comes in in recurrent supervised. Implicit here but make it explicit. 
...
multiplication that can be performed between a vector and a \glossary{scalar}, which is just a single number like $2$. 
...
vector} with $n$ components, one for each activation value. For example, if we index the nodes of the feed-forward network in Fig. \ref{ffRecurrent} (Left) from the bottom to the top and left to right (as in Fig \ref{labelledNets}), then that networks' activation vector is $(-0.3,0.6,-0.3,0.5,-0.2,0.6,0.2)$. This 
is a vector in a 7 dimensional vector space. A vector space of activation 
vectors is called an \glossary{activation space}. The feed-forward network in Fig. \ref{ffRecurrent} (Left)  network has a 7 dimensional activation space.
...
We can also talk about vectors of weights, or \glossary{weight vectors} which exist in \glossary{weight spaces}. The feed-forward network in Fig. \ref{ffRecurrent} has 12 weights. The strengths of those weights is given by the vector 
\begin{equation*}
...
It can also be useful to talk about a \glossary{fan-in weight vector} (the list of weight strengths for the set of weights attaching to a node), and a \glossary{fan-out weight vector} (the list of weight strengths for the set of weights exiting a node). A version of the networks in Fig. \ref{ffRecurrent} with zeroed out activations, labeled node indices, and weight strengths  is shown in Fig. \ref{labelledNets} below. Some sample weight vectors for these networks are:
\begin{eqnarray*}
...
For the feed-forward network in Fig. \ref{labelledNets}, we can begin with a matrix for the full network, which illustrates some of its structure.
\begin{equation*}
   \left( 
   \begin{array} {c|c|c}
...
However in practice it is natural to focus on the non-zero blocks, and to treat each set of connections between a pair of layers as its own matrix. For example, for the input to the hidden layer in  Fig. \ref{labelledNets} (Left):, we focus on this block of weights:
\[
\begin{pmatrix}
 1  &   0.7  & 2  \\
 -2  &   -1  & 2.1
