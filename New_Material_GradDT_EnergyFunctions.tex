...
We can now define dynamics using these energy functions. We can follow the gradient up or down (gradient descent or ascent) and in doing so we define a dynamical system on the state space ``beneath'' the energy function.

This is where the hill and valley metaphor comes from. Gradient dynamics using gradient descent can be thought of as going to low points in an energy landscape.  This is what attractor dynamics and attractor networks usually refer to.

There are two applications, one regarding performance, the other learning. In performance, we have recurrent networks and attractor networks. Another application is in learning, where gradient descent is used for optimization.

One special kind of energy function is a Lyapunov function, which is a special function where the gradient dynamics are well behaved, always having attractors.\footnote{Not to be confused with Lyapunov exponents, which are used to identify chaotic orbits. Lyapunov was a smart guy!}.  What Hopfield basically did is prove his networks had memories by defining a Lyapunov function. As Hopfield says:
...
in figure \ref{F:exampot} (via the arrows).
