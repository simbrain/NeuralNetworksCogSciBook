...
\begin{figure}[h]
\centering
\accessibleimage[scale=.4]{./images/TablesAndNets.png}{Three supervised-learning examples. Each pairs a small input-target table with a network: a one-to-one network, a two-to-one network, and a two-to-two network.}
...
In this chapter we focus on feed-forward networks, which can be thought of as implementing vector-valued functions (such networks are often referred to as ``MLP's'' or ``Multilayer Perceptrons'', in reference to earlier work by Widrow and others; see chapter \extref{ch_history}). A labeled dataset is essentially a specification for a vector-valued function we'd like our compatible network to implement. Given an input vector $\mathbf{x}_r$ in a labeled dataset, we want the network to produce an output vector $\mathbf{y}_r$ as close as possible to the corresponding target vector in that dataset.
...
Below we refer to ``errors''  and ``overall error'', which are discussed in greater detail in section \ref{sect_error}. Roughly speaking individual errors say how far away the outputs produced by a network relative to an input are from the target values for that input, and overall error combines these errors together into a single number for all the nodes and all the rows of the dataset.
...
We will see that this can be visualized in a geometric way, as ``gradient descent'' to a low point on an ``error hypersurface.''
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.4]{./images/letterClassification.png}{Feed-forward classifier for letter patterns. A grid-like input layer feeds hidden layers and output units, with the active output identifying the presented letter.}
...
\begin{figure}[h]
\centering
\raisebox{-0.5\height}{\accessibleimage[scale=.35]{./images/linearRegressionTable.png}{Table of hours studied and SAT scores, showing scores generally increasing with study hours.}}
\hspace*{.4in}
\raisebox{-0.5\height}{\accessibleimage[scale=.4]{./images/linearRegression.png}{Scatterplot of SAT score versus hours studied with an upward-sloping linear regression line.}}
\hspace*{.4in}
\raisebox{-0.5\height}{\accessibleimage[scale=.3]{./images/2Node_Network.png}{Two-node linear network. An input node labeled Hours Studied connects by a weighted arrow to an output node labeled SAT Score.}}
...
\begin{figure}[h]
\centering
\accessibleimage[width=0.9\textwidth]{images/visualizeClassification.png}{Classification in input space. A one-input network has a threshold point that separates output classes zero and one; a two-input network has a diagonal decision boundary that separates the two classes.}
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.4]{./images/visualizeRegression.png}{Regression in input-target space. A one-input linear network maps points to a fitted line; a two-input linear network maps points to a fitted plane.}
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/ErrorComputation_1.png}{Sum-of-squared-error calculation for one output node. A table of targets and outputs is subtracted row by row, the small errors are squared, and their sum gives low SSE.}
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/ErrorComputation_2_low.png}{Sum-of-squared-error calculation for two output nodes with outputs close to their targets, producing a low total error.}
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/ErrorComputation_2_high.png}{Sum-of-squared-error calculation for two output nodes with outputs far from their targets, producing a higher total error.}
...
Figure \ref{error_surfaces} shows error surfaces for two cases that we can visualize. On the left we have an error surface for a 1-1 network where we are only adjusting a single weight. As we change that weight, SSE will change too. On the right we have a 2-1 network where we are adjusting two weights. Again, as the two weights are changed so will the SSE. Notice that in each case the error surface has a single minimum point, which turns out to be convenient: our goal, after all, will be to find that lowest point, the configuration of weights where overall error is lowest.  But we are not always lucky enough to get such a surface, as we will see.
% Convexity, when this is the case

\begin{figure}[h]
\centering
\accessibleimage[scale=.5]{./images/ErrorSurfaces.png}{Error surfaces for supervised learning. With one adjustable weight the surface is a one-dimensional curve; with two adjustable weights it is a three-dimensional bowl-shaped surface.}
\caption[Jeff Yoshimi.]{(Left) error surface for a 1-to-1 network where only the weight is adjusted. (Right) Error surface for a 2-to-1 network where the two weights are adjusted. In each case, the error surface also depends on the training data (the tables in figure \ref{tables_nets}). }
