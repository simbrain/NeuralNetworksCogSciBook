...
\item[Antipattern] Intuitively, the antipattern of a pattern $p$ is the pattern formed by ``flipping'' every value in $p$.  For a binary pattern we swap 0's and 1's. For a bipolar pattern we swap -1's and 1's. Examples: the antipattern of $(0,1,0)$ is  $(1,0,1)$. The antipattern of $(-1,1,-1)$ is  $(1,-1,1)$. Antipatterns are a kind of spurious memory.
...
\item[Backpropagation] (Synonym: Backprop) A supervised learning algorithm that can train the weights of a multilayer network using gradient descent. Can be thought of as an extension of Least Mean Square methods for multilayer networks.

\item[Bag of words] A representation method that associates a document with a vector of word frequencies, where each entry of the vector corresponds to a word and the value of the entry corresponds to the number of times that word occurs in the document. The underlying idea is that the word usage frequencies would capture the (semantic) content of a document. The order and grammatical information of the words in the document is disregarded, hence the term ``bag''.  Forms of document embedding, as contrasted with token or word embeddings, are more common with neural networks.
...
\item[Competitive learning] A form of unsupervised learning in which output nodes are trained to respond to clusters in the input space.
...
\item[Confabulation] The generation of plausible but misleading or false content by a model.
...
%\item[Controlled process] A cognitive process that requires attention for its execution, and is relatively slow. Examples include solving math problems, doing homework, or performing an unusual task that you have not practiced.
...
\item[Crosstalk] A phenomenon where training patterns interfere with one another when training a neural network to perform some task.
...
\item[Dataset] Any table of numerical values that is used by a neural network, or that will be used by a neural network after preprocessing. (This is not a standard definition, but one stipulated in this text). Input, output, target, and training datasets are specific types of tables used in specific ways by neural networks.

\item[Data wrangling] (Synonyms: data munging, preprocessing) The process of transforming data into a form usable by a neural network. Encompasses obtaining, cleaning, imputing, coding, and rescaling data.
...
\item[Error surface] The graph of a function from parameter values of a network to error values of an associated error function. Each point on an error surface corresponds to different parameters (usually weight values and biases) of a network. Gradient descent finds minima on an error surface, where error is relatively low.
...
\item[Excitatory synapses] Synapses where an action potential in a pre-synaptic neuron triggers the release of neurotransmitters that then increase the likelihood of an action potential occurring in the post-synaptic neuron.
...
\item[Feature vector] A vector of numerical values representing relevant properties or features of an item. For example, a feature vector for an image of a piece of wood might represent its color, texture, and the type of knot it contains.

\item[Feed-forward network] A network comprised of a sequence of layers where nodes in each layer are connected to neurons in subsequent layers. Contains no recurrent connections. Skip or residual connections that allow information to bypass one or more layers can also be used. Commonly used in supervised learning tasks, such as classification and regression.
...
\item[Fine-tuning] Additional training performed on a model with a relatively small learning rate to adapt it for a specific domain or task. Fine-tuning nudges the weights to better capture new domains while preserving the broad linguistic competence and background knowledge gained during pretraining. Giving base models personalities is commonly done with fine-tuning.

\item[Firing rate] The number of spikes (action potentials) a neuron fires per unit time. Usually measured in hertz, that is, number of spikes per second. A higher firing rate corresponds to a more ``active'' neuron.
...
\item[Foundation models] Base models, often built by large companies, that then serve as a foundation for later stages of fine-tuning. Once the computationally expensive work of pretraining is complete, many different specialized model versions can be built from a good foundation model.
...
\item[Hallucination] A type of error made by a model where it describes something based on statistical patterns in its training data, but where that thing does not actually exist. This is usually something that seems like it \emph{ought} to exist, given the training data, but that does not in fact exist. A type of generalization error. Example: an LLM produces a reference to a paper that does not exist, by an author who does exist.
...
\item[Hidden unit space] The space of activation vectors for a network's hidden units.
...
\item[Inhibitory synapses] Synapses where an action potential in a pre-synaptic neuron triggers the release of neurotransmitters that then decrease the likelihood of an action potential occurring in the post-synaptic neuron.
...
\item[Localist representation] A representation scheme where activation of individual neurons indicates the presence of an object. Example: activation of neuron 25 indicates the presence of my grandmother.
...
\item[Logits] Scores produced by a classification output layer before conversion to probabilities. For a softmax output layer, these are the weighted inputs, including biases. Logits can be any real numbers and need not sum to one; softmax converts the vector of logits into a probability distribution.
...
\item[Orthogonal] Two vectors are orthogonal to each other if their dot product is zero. One-hot vectors are orthogonal. They are widely separated in input space and tend not to produce crosstalk in learning tasks.
...
\item[Pool shape] The dimensions of a pooling window in a convolutional neural network, consisting of height and width (e.g., $2 \times 2$ or $3 \times 3$), but no depth. The pooling window slides only in the spatial dimensions.
...
\item[Prefrontal cortex] The frontmost part of the frontal cortex, which is involved in executive function, decision making, and planning. It is also thought to have an attractor-based structure that supports the operation of working memory.

\item[Preprocessing] The process of transforming data into a form usable by a neural network. Compare data wrangling.
...
\item[Reinforcement learning] A form of learning in which a system learns to take actions that maximize reward in the long run.   Actions that produce rewards, or actions that lead to actions that produce reward, are reinforced in such a way that agents learn to obtain rewards and avoid costly situations.  In humans, associated with circuits in the brain stem and basal ganglia.  Sometimes treated as a third form of learning alongside supervised and unsupervised learning.

\item[Reinforcement learning with human feedback] (Acronym: RLHF) A technique for training language models where human evaluators provide feedback on outputs, which is then used to train a reward model that guides further optimization via reinforcement learning. Typically involves three stages: supervised fine-tuning on demonstrations, training a reward model from human preference comparisons, and using reinforcement learning to optimize the language model against the reward model. RLHF is commonly used in post-training to align language models with human preferences and values, making them more helpful, harmless, and honest. This technique was crucial in transforming base models into chat models with coherent personalities.

\item[ReLU] A linear activation function that is clipped at  0. Its activation is 0 for weighted inputs less than or equal to 0, and it is equal to weighted inputs otherwise. It is a popular activation function for deep networks. Note that ``relu'' is short for ``rectified linear unit''.
...
\item[Reservoir computing] A recurrent-network approach in which the internal connections remain fixed and only the output layer is trained.
...
\item[Self-supervised learning] A form of supervised learning where targets or labels are not provided separately but are derived from the training data itself. This allows training data to be generated without explicit annotations. See auto-regression.

\item[Shape] An ordered list giving the size along each axis of an array. For a matrix, shape gives the number of rows followed by the number of columns: $(1000,512)$ means 1000 rows and 512 columns. A batch of 100 RGB images, each with 25 rows and 25 columns, has shape $(100,3,25,25)$ in batch, channel, row, column order. In this book, we use shape notation for matrices and higher-rank arrays; programming libraries also report shapes for scalars and vectors.

\item[Sigmoid activation function] An activation function whose value increases monotonically between a lower and upper bound. As the input goes infinitely far in the positive direction the value converges to the upper bound. As the input goes infinitely far in the negative direction the value converges to the lower bound.

\item[Simple recurrent network] A recurrent neural network with a context layer that stores the previous hidden-layer activations.

\item[Skip connection] (Synonym: residual connection) A connection between two nonadjacent layers of a feed-forward network that carries an activation vector past at least one intervening hidden layer. Skip connections are an important part of transformer architectures where they are part of a structure called the ``residual stream.''
...
\item[Somatosensory cortex] Frontmost region of the parietal lobe that houses a somatotopic map of the body parts and processes tactile information from the body.
...
\item[Sparsity] The sparsity of a matrix is a number between $0$ and $1$ obtained by counting how many zero entries the matrix has and dividing by the total number of entries. A matrix with no zero entries has a sparsity of $0$ and a matrix with all zero entries has a sparsity of $1$.

\item[Spike] A discrete event that models the action potential for a neuron.

\item[Spiking neuron] A neuron model that represents neural signaling as discrete spikes, or action potentials, occurring at particular times.
...
%\item[Truth table] A table whose columns are the inputs and outputs of a Boolean function.
...
\item[Winner-Take-All] A pool of nodes structured (often with mutually inhibitory connections) so that the node receiving the highest weighted inputs ``wins'' and becomes active, while the other nodes become inactive.
