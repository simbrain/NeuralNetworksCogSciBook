...
As noted in chapter \extref{ch_history}, we are now living in what we call the ``age of generative AI.'' In 2017 Google introduced the \textbf{transformer architecture}---discussed in chapter \extref{ch_transformers} and summarized below, which was subsequently adopted and extended by OpenAI and many other groups in academia and industry \cite{vaswani2017attention}. The power of this architecture became widely known with the public release of OpenAI's ChatGPT in late 2022. ChatGPT had the fastest adoption rate of any software in history, and marked another shift in the history of neural networks and AI broadly. In this period, it became common to train extremely large models on vast amounts of data. The resulting neural networks could be used to generate convincing text, images, audio, and video, hence the term \glossary{Generative AI}.\footnote{Eli Olson created a \href{https://bluelijah.github.io/Inside-LLMs/}{\underline{page with visuals}} that is loosely tied to this discussion.}
...
In this section we will work out a very basic understanding of how LLMs work. The basic idea is to use next-token prediction in a special ``recursive way'' that ends up producing reasonable chat interactions. In this discussion we will only focus on understanding the inputs and outputs to the feed-forward network. Other chapters discuss more of the details of what happens inside.
...
Whether or not they do, they certainly come much closer than any earlier attempt did, so this is part of why we are in such a new and different age today. Few think current LLMs are conscious, but they are suggestive of a future AI with similar structure and more multimodal capabilities (vision, hearing, etc), agentic capabilities to do things, more persistent memories, etc. A review of the topic is in \cite{chalmers2023llmconscious}. 
...
world and don't have bodies, so they can't think like we do, or deal with complex situations in the way we do. (This is consistent with stochastic parrot arguments). Dreyfus pointed out how problems in everyday life involve an infinite number of potentially relevant facts, but computers can only search a finite set of possibilities. For example, SHRDLU only operates in a microworld of shapes, and ELIZA can only handle simple conversations that mimic therapy.\footnote{The frame problem is a closely related but more formal problem in logic-based AI. It concerns how to represent what does or does not change in an object from one time to another, without having to list every possible fact.} Dreyfus posed this as a dilemma:

\begin{quote}
Either AI systems try to examine every relevant factor in dealing with a situation, but that is impossible because there are infinitely many possibly relevant factors.

Or AI systems examine a limited set of factors, but then they won't do as well as an intelligent human.

Either way, they are inadequate. Humans with their insight automatically know which factors are relevant.
\end{quote}

In earlier days, one could illustrate this old argument by presenting AI systems with weird situations. Yoshimi used to do a demo in class, where he would ask strange or unusual questions to AI systems and chatbots of earlier years. Here is a reconstruction of such an exchange with Mitsuku, a chatbot that won the Loebner Prize \cite{wikiKuki2026} (the original transcripts are lost)\footnote{I did find this example with Eliza from a course in 2015: ``(Me) How are you today Eliza? (Eliza) Why are you interested in whether or not I am today Eliza?''}:
...
LLMs may encode a kind of \emph{world model}. After all, the internet was written by people with embodied experience of the world. Some review of the discussion about world models in LLMs is in \cite{chalmers2023llmconscious}.
...
\begin{quote}
Either connectionism cannot do some important things classical architectures can do (compositionality, systematicity, productivity), in which case connectionism fails as a theory of the mind.

Or connectionism can do those things, in which case connectionism is merely a story about how classical structures are implemented.

Either way, connectionism does not provide a viable alternative to classical AI.
\end{quote}

So where do LLMs stand? They seem clearly to be systematic, productive, and compositional. They can say things in multiple ways, they can produce indefinitely many statements, they are aware of how the ``Lego blocks'' of language fit together and how the meanings of the whole derive from the meaning of the parts. They can handle any test comprehension sentence or linguistic aptitude test that Fodor and Pylyshyn ever came up with. 
...
To get a first-pass sense of the concern, consider the ``Sorcerer's apprentice'' and old poem by the German poet Johann Wolfgang von Goethe (you may also know it from the Disney film). Here is Wikipedia's summary of the original poem:
