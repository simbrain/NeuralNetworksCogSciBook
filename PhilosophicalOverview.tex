% One-off standalone document assembled in part from existing chapter material for use in a history and philosophy of cognitive science class.
...
This creates a strange situation, noted in several other chapters. Even though we have set up these systems and ``grown'' them, we do not fully understand how they work on the inside. When else in the history of humanity have we built something we don't really understand? The field of mechanistic interpretability (chapter \extref{ch_mechinterp}) is emerging as an area which develops tools to understand what is going on inside LLMs, but they have not finished their work. The problem remains open. This is is linked to some of the philosohpical issues surrounding generative AI, like the question of how to regulate them and ``grow'' them in a safe way. More on this below.
% interpretabilty more generally
...
% Support with philpapers numbers
Despite how new they are, the philosophical literature surrounding LLMs is massive. 

\subsection{Stochastic Parrot or Genuine Intelligence?}

% Link to Chomsky and symbolic approaches
...
Others argue that LLMs are doing more than recalling patterns from their training data. On this view, LLMs are doing the same kind of computations humans do: assembling relevant information in response to situations and producing emergent genuine intelligence in the process.  There is quite a bit of empirical data now that supports this view, by showing that LLMs often behave like humans do.
% Summarize that data

Much of the work here is in the details of how LLMs work (hence mech interp is important) and in how they compare to humans, and this work is very active and ongoing. See section X TODO of chapter X.

\subsection{Turing Machines and Consciousness}

% Picture of chinese gym
% Eric: maybe ditch octopus test because confusing
...
Whether or not they do, they certainly come much closer than any earlier attempt did, so this is part of why we are in such a new and different age today. Few thing current LLMs are conscious, but they are suggestive of a future AI with simmilar structure and more multimodal capabilities (vision, haering, etc), agentic capabilities to do things, more persistent memories, etc.  A useful review of the topic is in  \cite{chalmers2023llmconscious}. 
...
world and don't have bodies, so they can't think like we do, or deal with complex sitautions in the way we do. This is consistent with stochastic parrot arguments. One way the old argument went would be to present LLMs with weird situations.   I used to class ask strange or unusual
...
In response, LLM enthusiasts will note that current chat bots can deal quite well with weird situations. They seem to have solved the frame problm.But current
...
% Check all mechinterp
Remember, we don't program neural networks, we \emph{grow} them. So we don't know just what they are doing yet (maybe we will someday we will with mech-interp, but that day has not come yet). Again, we grew these things that we don't understand. This is part of why alignment is such a big deal.
