...
% TODO: Add normalization and min-max scaling. Technically normalizing is dividing each entry by the L2 norm of the column (square root of summed squares). This takes the vector corresponding to the column (not something we often think about, this is in the rowspace not the col space, i.e. a data space rather than say an input space) and places it on the unit sphere, a unit sphere encoding.  But "normalize" often means rescale vaguely and can thus refer to any of these methods. Min-max is dividing each entry of the column by the max value (for positive values) or by recentering using min and dividing by max-min more generally
