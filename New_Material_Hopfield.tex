...
$0$ if $x(t)$ is not positive and it is $1$ if $x(t)$ is positive.
...
\subsection{Types of Update}
   ...
   The rules \eqref{E:hopinput} and \eqref{E:hopIO} can be applied synchronously
...
% Do these have stationary distributions? But they are unusual in that they do not have _unique_ stationary distributions. 
% These are so simple no need to take eigenvalues etc.
% For example, for inputs 0,0 we'd get 0,0 as the only stationary distribution, but for inputs 1,1 we'd have 1,0 and 0,1 as stationary distributions. 
...
figure \ref{F:TwoNodeHopfield}.  The directed edge pointing away from a vertex 
...
oscillate between the states $(0,0)^T$ and $(1,1)^T$.
...
easily computed.  They are shown in figure \ref{F:discHoppot}.  The directed 
...
are shown in figure  \ref{F:TwoNodeHopfieldSingle}.  We can obtain the digraph 
for $F_1$ by using the formula for $F_1$ or by redirecting the arrows in figure 
\ref{F:discHoppot} so that only $y_1$ changes.  This is shown on the left in
figure \ref{F:TwoNodeHopfieldSingle}.  Similarly we can obtain the digraph for 
$F_2$ from the formula or by redirecting the arrows in figure \ref{F:discHoppot} 
so that only $y_2$ changes.  This is shown on the right in figure  
...
\end{figure}
  ...
  In addition figure \ref{F:TwoNodeHopfieldSingle} shows that $F_1$ and $F_2$ 
...
\end{pmatrix}
 \right)
\end{eqnarray*}
The digraphs for these two functions are shown in figure  
\ref{F:TwoNodeHopfieldSequence}.  We can obtain the digraph for $F_{12}$ by
seeing where states go in the digraph for $F_2$ in figure 
...
digraphs for $F_1$ and $F_2$ in figure \ref{F:TwoNodeHopfieldSingle} into a
single digraph.  This union of digraphs is shown in figure 
...
$(1,1)^T$ and the system starts out in $(1,0)^T$ or $(0,1)^T$ then it will
stay in its starting state, otherwise it eventually reaches either $(1,0)^T$ or 
$(0,1)^T$ with probability of $1/2$, and stays there.}
...
randomly selected to be updated.  Figure \ref{F:TwoNodeHopfieldStochastic} shows 
...
An actual use is to recommend movies. Put in a person's set of preferred movies. Now put in a subset. The network figures out the rest. But feed-forward models can do this, and probably better, so they fell out in engineering.
