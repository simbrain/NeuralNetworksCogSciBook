...
Now we can describe the LMS rule, and verify that it causes the weights to change in such a way that the sum of squared errors (SSE) gets lower over time.
...
\section{Appendix: Finding A Linear Decision Boundary Manually}
...
\subsection{Geometric intuition about the decision boundary}

In a neural network with two inputs $x_1$, $x_2$ and one output, the weighted input value $n_3$ for the output node is given by the equation:
\[
n_3 = w_{1,3} x_1 + w_{2,3} x_2 + b_3
\]
The threshold for classification can be visualized as a horizontal plane, typically at $n_3 = 0$ (as in our example), which intersects the surface defined by the weighted sum of the inputs. The surface defined by $z_3 = w_{1,3} x_1 + w_{2,3} x_2 + b_3$ is a plane in 3D space, where the weights $w_{1,3}$ and $w_{2,3}$ can be viewed as spanning the plane. The plane is spanned by two vectors: $(w_{1,3}, 0, 1)$ and$(0, w_{2,3}, 1)$. The bias $b_3$ shifts this plane vertically along the $z$-axis without affecting its orientation, while the weights determine the plane’s tilt.
% Above is regression surface

The decision boundary can be visualized as the intersection of the threshold plane, determined by the threshold value $\theta$ (see the activation function chapter), and a surface defined by the weights and bias. See figure \ref{decisionLineSurface}, for the case of a threshold of 0. As you can see visually in the image, inputs below the decision boundary will produce a net-input which is below threshold, while those above the decision boundary will produce a net-input which is above threshold.

% This is actually the link between regression and classification. Where the regression surface intersects the threshold is our decision boundary!  Also note 3 state variables and 3 paramters.
