...
% https://youtu.be/H-4bmOxiKyU?t=177.  Each token is a person. Everyone has all three vectors.  They can yell out a query and it hits all the keys. She then takes values weighted by the key-query pairs.  Thse are like heat-maps for each person, or patch, or token. This heat map is like an attention template. It then gets multiplied to get the "attended" values. Like take everyone's value vector and weight it by the heat map attention filter. Then you have a new, fourth thing, my sense of what is important in the room. Each person has one. That matrix of attended value vectors is I think what the output of the attention mechanism is.
