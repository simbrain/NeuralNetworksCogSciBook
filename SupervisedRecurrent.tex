...
\end{description}
Even though this is framed in terms of single vectors and sequences of vectors, the boundaries between these cases can be fuzzy: a sequence-to-vector model, for example, might really involve a small sequence of vectors as input (like a brief text prompt) and a much longer sequence of output vectors. Thus, one might also think of these as involving: a long response to a short input; a short response to a long input; and equal-length responses. (Interestingly, large language models like ChatGPT use a more purely feed-forward approach, taking a large set of input words and producing a single predicted next word; see chapter \extref{ch_transformers}.)
% Forecasting
% More examples

\begin{figure}[h]
\centering
\frame{\accessibleimage[scale=.4]{images/karpathy_rnn.png}{Three recurrent-network sequence-learning patterns. One-to-many expands one input into several outputs; many-to-one condenses a sequence into one output; many-to-many maps each input in a sequence to an output.}}
...
The structure of an SRN is shown in figure \ref{SRN_Structure}. It is basically a standard 3-layer feed-forward network trained using backpropagation, with some special machinery for processing temporal context. The special machinery is the ``last hidden state'' portion of the input layer, which is always set to the hidden layer activation vector of the last time step (in the first time step it is usually just set to the zero vector).\footnote{ As McClelland says, ``The beauty of the SRN is its simplicity. In fact, it is really just a three-layer, feed-forward back propagation network. The only proviso is that one of the two parts of the input to the network is the pattern of activation over the network's own hidden units at the previous time step'' \weburl{ https://web.stanford.edu/group/pdplab/pdphandbook/handbookch8.html}.} It is a ``copy-back'' of the hidden  layer. Otherwise it is like another part of the input layer, fully connected to the hidden layer. Thus at any time the full input to the network is the current input, \emph{plus some temporal context}. It's a bit like  someone saying ``Good times!''. At the moment you hear them say ``times!'' you have some memory of them having just said ``Good''. You  hear ``times!'' in the context of ``Good''. This allows you to distinguish ``good times'' from ``bad times'' from ``crazy times'', etc.\footnote{This idea occurs in philosophy in the work of Edmund Husserl and others who claimed that human experience essentially involves ``time consciousness'', which in turn includes an awareness of what has just-passed and what is about to come (and note that SRNs are usually trained to predict one step in the future). See \weburl{https://plato.stanford.edu/entries/consciousness-temporal/}.}$^,$\footnote{But note it's not the past input that is remembered, it's the past hidden state. That hidden state is influenced by the past input, \emph{and} earlier hidden states. Thus there is a recursive relationship here that allows the temporal influence to extend arbitrarily far back in the past, though the influence is strongest in the recent past. As an analogy, consider a giant pot of soup to which new ingredients are added each day. At any time it tastes most like the current ingredients, but there are always hints of past ingredients in there, with the more recent ingredients more prominent than ingredients added in the distant past.}
...
\begin{figure}[h]
\centering
\accessibleimage[scale=.6]{./images/srn.png}{Simple recurrent network diagram. Current-state input nodes and copied last-hidden-state nodes feed a hidden layer, which produces predicted next-state outputs; arrows from the hidden layer back to the last-hidden-state nodes carry temporal context.}
...
In the toy example being considered here, the network is being trained on a ``bouncing one'' pattern. For example, if we enter $(1,0,0)$, the SRN should predict that $(0,1,0)$  comes next. But notice that the vector $(0,1,0)$ is ambiguous. It will predict \emph{different} outputs depending on when we present it. It predicts  $(1,0,0)$ after  $(0,0,1)$, but  $(0,0,1)$ after  $(1,0,0)$. How can the network do this? The answer is: by using the context  layer. The last hidden state after seeing $(1,0,0)$ is different than it is after seeing $(0,0,1)$. This allows the network to differentiate the same input, $(0,1,0)$, in different temporal contexts.
...
\begin{figure}[h]
\centering
\frame{\accessibleimage[width=0.5\textwidth]{images/BPTT.png}{Unrolled recurrent network across three time steps. Repeated input, hidden, and output layers are connected forward through time; backpropagation-through-time error arrows run backward from later to earlier hidden states.}}
...
In a now-classic blog post, Andrej Karpathy describes some of these applications.\footnote{\weburl{http://karpathy.github.io/2015/05/21/rnn-effectiveness/}. Note that Karpathy makes use of LSTMs; see below.}  For example, he trained a neural network on a bunch of Shakespeare\footnote{The data is here: \weburl{http://cs.stanford.edu/people/karpathy/char-rnn/shakespear.txt}.}  (with words coded as vectors, of course) and then ran the RNN, which produced it's own version of Shakespeare \cite{karpathy2015unreasonable}. Here is a sample:
...
Karpathy got these supervised recurrent networks to do other cool things:  for example, he generated a fake wikipedia page, fake source code, and a fake math paper \cite{karpathy2015unreasonable}. A fragment from the artificially generated mathematics paper is shown in figure \ref{fakeMath}. These techniques have even been used to generate a fake script for a movie, which was then actually produced!\footnote{The move is called ``Sunspring'', and can be viewed here: \weburl{https://www.youtube.com/watch?v=LY7x2Ihqjmc}. The opening shows a list of all the other movie scripts that were used to train the network to produce its script \cite{newitz2016movie}}

\begin{figure}[h]
\centering
\frame{\accessibleimage[width=0.7\textwidth]{images/recurrentMath.png}{A textbook excerpt defining a recurrent network mathematically, with equations for the hidden state, output, and recursive dependence on prior states.}}
