...
% Eventually it might make sense to make LLM's a separate chapter
...
\caption[Jeff Yoshimi.]{Schematic of backprop through time. The actual network that we are training is on the left: a feed-forward network with a recurrent weight layer in the middle. The unrolled network is on the right. This network can learn a sequence of three input-target pairs. We train the unrolled network to produced target values in response to current inputs \emph{and} to previous hidden layer states. When training is done, the changes to the weight matrices (the arrows) are added together and so we have ``rolled the network back up'' to the network on the left.}
\label{bptt}
\end{figure}
...
% Maybe this should be a new chapter.

% I"m liking "Machine learning" channel, e.g https://youtu.be/1IKrHh2X0F0 (around 10 min)

% https://youtu.be/Smav86u60FM. Going from scalar to token operators, and nice analogy to convolution. Also fantastic on attention with graphics at 11:33
% https://youtu.be/H-4bmOxiKyU?t=177.  Each token is a person. Everyone has all three vectors.  They can yell out a query and it hits all the keys. A person takes values weighted by the key-query pairs.  These are like heat-maps for each person. The maps then get multiplied to get the "attended" values. // Take everyone's value vector and weight it by the heat map attention filter. Then you have a new, fourth thing, my sense of what is important in the room. Each person has one. That matrix of attended value vectors is I think what the output of the attention mechanism is.

% About KV Cache, a perspective that helps see how it actually works
% https://youtu.be/80bIUggRJf4?t=176

% From gpt-3 tech paper: "tokens. Note that during training, datasets are not sampled in proportion to their size, but rather datasets we view as higher-quality are sampled more frequently, such that CommonCrawl and Books2 datasets are sampled less than once during training, but the other datasets are sampled 2-3 times"

% Talk about how connectionist and comp neuro of this barely discussed, at least not yet

% https://data-science-blog.com/blog/2021/04/07/multi-head-attention-mechanism/
% On reps!! https://twitter.com/blader/status/1655992260448886789
% Given its prominence, there are now lots of sources, hard to keep up
% https://benlevinstein.substack.com/p/a-conceptual-guide-to-transformers?sd=pf
% Watch the whole karpathy thing
% https://www.youtube.com/watch?v=fMiYNrjDPyI
%!TEX encoding = UTF-8 Unicode https://writings.stephenwolfram.com/2023/02/what-is-chatgpt-doing-and-why-does-it-work/
%    The one above is great for unpacking next word pred. (And the remarkable thing is that when ChatGPT does something like write an essay what it’s essentially doing is just asking over and over again “given the text so far, what should the next word be?”—and each time adding a word. (More precisely, as I’ll explain, it’s adding a “token”, which could be just a part of a word, which is why it can sometimes “make up new words”.)
% It's like a massive average over human output.  (Cf. the jpeg analogy)
