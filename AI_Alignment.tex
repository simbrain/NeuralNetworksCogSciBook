...
% Pull ideas from PhilosophicalOverview, including Dave's comments / critiques on that document
% superintelligence: Summoning an alien species
...
% More back refs to other chapters 
% Discuss prompt alignment?

% Reduce length of example in 1.2 and 1.2. and 1.3. David is ok with that.


\glossary{AI alignment} is the field concerned with ensuring that the goals of artificial intelligence systems, such as large language models, are compatible with human goals and values. Compare the way the term ``alignment''  is used when someone joins a company or team and is asked whether they are ``aligned around the same goals'' or ``mission aligned.''  In AI alignment the question is whether the AI systems we build and the humans who are impacted by those systems are aligned around the same goals and values. The worry is that as AI systems become more capable, even small divergences between their objectives and human values could lead to unintended or harmful outcomes.  Some of these worries are practical and near-term, such as the risk of models optimizing for misleading behaviors; others are longer-term and existential, focusing on scenarios in which misaligned \glossary{superintelligent} systems (roughly, super-humanly intelligent systems) could act in ways fundamentally at odds with human survival or flourishing. Current research distinguishes \glossary{prosaic alignment}, which addresses the alignment of today’s machine learning systems, from \glossary{superintelligent alignment} (also ``ambitious alignment''), the more speculative challenge of aligning future, potentially transformative AI. A central technical challenge of the field is thus whether such alignment can be reliably achieved at all, and this difficulty has led to the development of what is now commonly called \glossary{technical AI alignment}.\footnote{Earlier discussions often used broader labels such as \emph{AI safety}, before more specific distinctions were drawn.  The phrase \emph{AI control} was also used in some early work as a near-synonym of alignment, though it now refers to a narrower set of problems within the field.}
% For example: https://www.alignmentforum.org/posts/Hy6PX43HGgmfiTaKu/an-ambitious-vision-for-interpretability


% RLFH. Initially there is a person grading a bunch of stuff. That is used to create a reward model.  That is a map from goodness [chats?] to reward. Then you fine tune an existing chat model against that reward model.  // Generate  a sample chat. The model completes. Find the reward from reward model. Then backprop the reward through. Then iterate until you get mostly friendly answers, or higher reward on the reward model.  // They do  goodheart and get high scores and sycophancies.  // What is the range of things they use this with? Not just politeness.  Is the range of things they use their reward. Anthropic has stuff on this. See "helpful, harmless, honest, " the three H's.  But there is a richer story. They have a whole constitution for every model. This is who you are, this is what you care about it. It's a ton of stuff. 
...
% Maybe the technical pipeline bit should be a separate section
...
%
%
%As it predates modern LLMs, AI alignment in fact predates \emph{generative AI} as we now
...
%
% JY: The argument below shows why AI might take over even if given a
...
several sizes here: \weburl{https://huggingface.co/openai-community}.} The
...
\weburl{https://gwern.net/gpt-3-nonfiction\#board-games}.}
...
here:
\weburl{https://gist.github.com/jareddk/2509330f8ef3d787fc5aaac67aab5f11\#file-hhh_prompt-txt}
}
...
\weburl{https://rr-lm-game.herokuapp.com/}. It is extraordinarily difficult, far
...
demonstration transcripts and was followed by further RLHF. This post-training
...
pre-training data and shaping chat model responses continues.
...
smart enough student model is a much easier task than hardcoding the correct
...
% PA basically means AI alignment where we bet on nothing big changes.  Others look at crazy new AI that might be possible in the future, prosaidc says let's assume it stays more or less as it is
Prosaic alignment refers to AI alignment concerned with AI built around
contemporary paradigms more or less like transformer-based LLMs. That is, it overwhelmingly
...
% In general, reward hacking means training a model to get a high reward in a way that you did not intend and did not, and you must patch afterwards. Famous example is the RL speedboat, and you do reward shaping. They want the boat to go around the track. It turns out the boat finds a nice little cluster of reward bumps and goes in a tight circle and gives up on winning the race. High reward.  // Claude writing tests that are hard coded to pass or otherwise cheats.  // I am reminded of Goodheart's law. If you start optimizing against a metric it stops being a good proxy.
...
somewhat different goals than were intended for them to
...
\caption[ Originally due to Reddit user @Curious\_Evolver,
    \href{https://www.reddit.com/r/bing/comments/110eagl/the_customer_service_of_the_new_bing_chat_is/}{\underline{here}}.
...
all of a model's thoughts is a less powerful control technique than it seems,
