...
somewhat different goals than we had intended for them to
...
fasten onto some proxy nearby where we point them. As with the case of lying,
it has been observed that the models generally do know when they are
...
security is taken seriously and attack surface is thought about, we can at
...
If you are just concerned with \emph{detecting} correlates of bad behavior in
...
concepts in large amounts of data. If that is all we need to scan forward
passes for worrying behavior, then we can perhaps lean on this basic machinery.
...
evaluation or are in normal deployment \cite{needham2025know}. So, when you
come up with a clever honeypot scheme for AI control, you have to work around
this awareness on the models' part.

% CoT monitoring; faithfulness
Because reasoning models think out loud, we can just directly monitor those
chains of thought to see what they are about, including whether they are
