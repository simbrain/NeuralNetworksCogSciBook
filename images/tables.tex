...
To compute SSE we need to go through every row, compare each target with its corresponding output (each of these is one error), square the result, and then add these errors together and divide by the number of rows. So we have:
\begin{eqnarray*}
SSE =  (1-0)^2 + (- 1-0)^2 + (-1-0)^2 + (1- 0)^2 + (1-0)^2 + (1 - 0)^2 =  \\ 
1 + 1 + 1 + 1 + 1 + 1 = 6
\end{eqnarray*}
So for this network and this training set, $SSE = 6$. This is intuitively obvious since every output for every row is ``wrong'' by 1, and there are 2 outputs for each of 3 rows. And that's terrible!  Our network is doing a terrible job.
